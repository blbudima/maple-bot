...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (!ddescription) {
      return message.reply('Incorrect usage! Please supply your edited description.');
    }
    if (ddescription.length > 300) {
    ...
    Character.find(query, (err, docs) => {
      if (err) {
        console.error(err);
        message.reply('there was an error editing your description. Please try again.');
        ...
        Character.findOneAndUpdate(query, { description: ddescription }, (err, result) => {
          if (err) {
            console.error(err);
            message.reply('there was an error editing your description. Please try again.');
          } else {
